...
extern "C" {
    ...
    JNIEXPORT void JNICALL Java_com_mapzen_tangram_MapController_nativeScreenToWorldCoordinates(JNIEnv* jniEnv, jobject obj, jlong mapPtr, jdoubleArray screenPos) {
        assert(mapPtr > 0);
        auto map = reinterpret_cast<Tangram::Map*>(mapPtr);
        jdouble* arr = jniEnv->GetDoubleArrayElements(screenPos, NULL);
        map->screenToWorldCoordinates(arr[0], arr[1]);
    ...
    JNIEXPORT void JNICALL Java_com_mapzen_tangram_MapController_nativeAddGeoJson(JNIEnv* jniEnv, jobject obj, jlong sourcePtr, jstring geojson) {
        assert(sourcePtr > 0);
        auto source = reinterpret_cast<Tangram::ClientGeoJsonSource*>(sourcePtr);
